...
class RecurrentThread(Thread):
    def __init__(self, interval: float = 1.0, target = None, name = None, args = (), kwargs = None):
        ...
        if interval is None or interval <= 0.0:
            loop_entry = self.__LoopFunc_0
        elif _IS_WINDOWS:
            loop_entry = self.__LoopFuncWin
        else:
            loop_entry = self.__LoopFunc

        super().__init__(target=loop_entry, name=name)
    ...
    def __LoopFuncWin(self):
        ...
        while not self.__quit:
            ...
            next_wake += self.__inter
            sleep_dur = next_wake - time.perf_counter()
            if sleep_dur > 0:
                time.sleep(sleep_dur)
            else:
                # Timer overrun; resync to current time to avoid drift explosion
                next_wake = time.perf_counter()
